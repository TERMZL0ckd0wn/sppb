                ...
                <div class="form-container max-w-xl mx-auto">
                    ...
                    <form action="{{ route('teacher.store') }}" method="POST" class="space-y-6">
                        ...
                        <div class="form-row">
                            ...
                            <select id="department" name="department" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm px-3 py-2">
                                <option value="">-- Pilih Jabatan --</option>
                                <option value="Jabatan Umum" {{ old('department')=='Jabatan Umum' ? 'selected' : '' }}>Jabatan Umum</option>
                                <option value="Jabatan Perniagaan" {{ old('department')=='Jabatan Perniagaan' ? 'selected' : '' }}>Jabatan Perniagaan</option>
                                <option value="Jabatan Hospitaliti" {{ old('department')=='Jabatan Hospitaliti' ? 'selected' : '' }}>Jabatan Hospitaliti</option>
                                <option value="Jabatan Teknologi Awam" {{ old('department')=='Jabatan Teknologi Awam' ? 'selected' : '' }}>Jabatan Teknologi Awam</option>
                                <option value="Jabatan Teknologi Maklumat" {{ old('department')=='Jabatan Teknologi Maklumat' ? 'selected' : '' }}>Jabatan Teknologi Maklumat</option>
                                <option value="Pengurusan" {{ old('department')=='Pengurusan' ? 'selected' : '' }}>Pengurusan</option>
                            </select>
                        </div>
